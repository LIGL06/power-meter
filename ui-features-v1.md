...
2. `ConsumptionCostChart`'s new per-point bar styling (`<Cell>` children on `<Bar>`) rendered no bar at all under this app's recharts version. Root-caused by temporarily swapping back the *original, untouched* pre-Phase-5 chart component against real non-zero backend data: the plain, single-fill `<Bar>` from Phase 3/4 renders **no visible rectangle either** — confirmed this is a pre-existing defect in this recharts version, not something Phase 5 introduced, and affects any real closed period with nonzero consumption, not just historical entries. At the time this shipped with a partial workaround (an explicit `shape` prop that rendered *something*, but with the wrong height) and a flagged follow-up. **Update, resolved in the bug-hunt pass after Phase 6**: instrumenting recharts' own source (temporarily, in `node_modules`, reverted after) showed `<Bar>`'s internal scale math was actually correct — only its own commit-to-DOM step was broken. Fixed properly by drawing the bars with a small `ConsumptionBars` component using recharts v3's own `useYAxisScale`/`usePlotArea` hooks directly, keeping `<Bar>` in the tree only (invisible) so the tooltip still has a registered series to read. Bars now render at the correct proportional height, live-verified including click-through to `/periods/:id`. See that pass's own notes for the full root-cause writeup.
...
## Post-Phase-6 bug-hunt pass
A full audit of the shipped UI against all four project docs (this file, `api-implementation-v2.md`, and both repos' `README.md`), done after all six phases above were live — the same kind of pass as `api-implementation-v2.md`'s own "Post-Phase-5 functionality audit," just aimed at correctness bugs rather than missing endpoints this time. Found and fixed, each confirmed live:

1. **`ConsumptionCostChart` bar rendering, properly fixed** (not just the Phase 5 workaround) — see the updated note under Phase 5 above for the root cause and fix.
2. **Onboarding's default anchor date used UTC "today," everywhere else in the same component compares against local "today."** For any timezone behind UTC (confirmed live at UTC-6), the UTC calendar date is ahead of the local one for several hours every day — so the form loaded with a default that (a) tripped the "you've backdated this" amber warning on first render for a date that was never touched, and (b) would have failed the very next fix below before the user changed anything at all. Fixed by using the same `todayISO()` (local date) the rest of the component already uses for the comparison.
3. **Onboarding had no upper bound on the anchor date at all.** Nothing stopped picking (or, via a bypassed native picker, programmatically setting) a *future* anchor date — the amber warning is written entirely about backdating and says nothing about this direction. A future anchor opens a period the server will never accept a reading against until that date arrives (`readAt` can't precede `period.startDate`, and also can't be in the future — a date past today satisfies neither), silently bricking the meter with zero warning. Fixed with a `max={today}` on the picker plus a zod `.refine()` rejecting it outright (belt-and-suspenders, matching the existing convention on the reading-entry date field for the identical "native `max` doesn't stop a programmatic value" reason).
4. **`PastBillsSection`'s "Exported (kWh)" field could never actually be submitted blank.** It's genuinely optional (only shown when the contract has exports at all), but react-hook-form's `valueAsNumber` produces `NaN` — not `undefined` — for a field left empty, and zod's `.optional()` only forgives `undefined`. Left blank, submission was silently rejected with a confusing validation error on a field the form never asked the user to fill in. Fixed by registering it with `setValueAs: (v) => (v === "" ? undefined : Number(v))` instead — the exact pattern already used (and working) for `AdminTariffsPage`'s `dacThresholdKwh`, just not applied here yet.
5. **No catch-all route.** Any unmatched path — a typo'd URL, a stale bookmark, a since-removed page — rendered a completely blank page with no nav, no error, no way back except the browser's own history. Added a `path="*"` route redirecting to `/` (authenticated) or `/login` (not), matching the redirect-based convention already used everywhere else in this router rather than inventing a dedicated "404 page" pattern that doesn't otherwise exist in this app.
6. **Deleting the last reading on a paginated page past the first stranded the view.** `ReadingHistoryPage` only shows the delete action on readings in the still-open period, which — once a period runs past 20 readings without closing (the norm for a 60-day period logged daily) — can land on page 2 or 3, not just page 1. Deleting the sole remaining reading there left the page showing a false "no readings logged yet" with the pager still reading "Page 3 of 2." Fixed by stepping back a page when a delete leaves the current page empty.
7. **Session rehydration treated a network error identically to an expired session** — explicitly flagged as "known, not fixed" in `api-implementation-v2.md`'s Phase 5 write-up and never revisited since. A page refresh while the server was briefly unreachable silently logged the user out locally (`clearTokens()` on *any* `/auth/me` failure), even though the stored token was still perfectly valid. Fixed using the exact `probeServerHealth()` pattern the contract/billing effects already use: only clear tokens when the server actively confirms the session is invalid; otherwise surface `serverUnreachable` so the app's top-level gate offers a Retry instead of bouncing to `/login`. Verified live end-to-end: killed the API mid-session, reloaded (got the "Can't reach the server" screen, not the login page), restarted the API, clicked Retry, landed back in the actual app state with the session intact.

Two further gaps found — real capabilities the backend already fully supports with no frontend UI at all, not bugs — are written up as `ui-features-v2.md` rather than fixed here.

Backend `npm test`/`test:e2e` unaffected (no backend changes this pass) and re-confirmed green (32 unit + 36 e2e). Frontend `tsc`/`vite build` clean throughout; `eslint` unchanged at 9 (same pre-existing findings, no new categories).
